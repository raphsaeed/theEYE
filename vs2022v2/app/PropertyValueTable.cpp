...
text format_f_number(std::uint16_t f_number)
{
    ...
    if ((0x0000 == f_number) || (SDK::CrFnumber_Unknown == f_number)) {
        ...
        if (modValue > 0) {
            ts << TEXTx('F') << impl::Round((f_number / 100.0), 1);
        }
        else {
            ts << TEXTx('F') << f_number / 100;
        }
    }
...
text format_iso_sensitivity(std::uint32_t iso)
{
    ...
    if (iso_mode == SDK::CrISO_MultiFrameNR) {
        ts << TEXTx("Multi Frame NR ");
    }
    else if (iso_mode == SDK::CrISO_MultiFrameNR_High) {
        ts << TEXTx("Multi Frame NR High ");
    }
...
text format_cameara_setting_save_operation(std::uint16_t camera_setting_save_operation)
{
    text_stringstream ts;
...
text format_cameara_setting_read_operation(std::uint16_t camera_setting_read_operation)
{
    text_stringstream ts;
...
text format_camera_setting_save_read_state(std::uint8_t cameara_setting_save_read_state)
{
    text_stringstream ts;

    switch (cameara_setting_save_read_state) {
...
text format_monitor_lut_setup(std::uint8_t monitor_setup)
{
    text_stringstream ts;
