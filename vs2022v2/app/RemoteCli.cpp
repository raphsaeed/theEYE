...
int main()
{
    ...
    cli::tout << major << "." << minor << "." << std::setfill(TEXTx('0')) << std::setw(2) << patch << "\n";
    ...
    while (true) {
        no = 0;
        if (std::regex_search(connectNo, smatch, cli::tregex(TEXTx("[0-9]")))) {
            no = stoi(connectNo);
            if (no == 0)
                break; // finish
   ...
   if (err == 0) {
    ...
    for (;;) {
        ...
        while (true)
        {
            ...
            //else if (action == TEXTx("f")) { /* ReleaseDevice */
            ...
            //    }
            //}
            else if (action == TEXTx("1")) { /* connect Remote */
            ...
            else if (action == TEXTx("2")) { /* connect MTP */
                if (camera->is_connected()) {
                    cli::tout << "Please disconnect\n";
                }
                else {
        ...
        if (SDK::CrSdkControlMode_Remote == camera->get_sdkmode())
        {
            ...
            while (true)
            {
                ...
                if (action == TEXTx("s")) { /* status display and device selection */
                ...
                else if (action == TEXTx("0")) { /* Return top menu */
                    if (camera->is_connected()) {
                        camera->disconnect();
                    }
                    break; // exit loop-B
                }
                else if (action == TEXTx("1")) { /* Take photo */
                    camera->capture_image();
                }
                else if (action == TEXTx("2")) { /* S1 Shooting */
                    camera->s1_shooting();
                }
                else if (action == TEXTx("3")) { /* AF Shutter */
                    camera->af_shutter();
                }
                else if (action == TEXTx("4")) { /* Continuous Shooting */
                    camera->continuous_shooting();
                }
                else if (action == TEXTx("5")) { /* Aperture. */
                    camera->get_aperture();
                    camera->set_aperture();
                }
                else if (action == TEXTx("6")) { /* ISO */
                    camera->get_iso();
                    camera->set_iso();
                }
                else if (action == TEXTx("7")) { /* Shutter Speed */
                    camera->get_shutter_speed();
                    camera->set_shutter_speed();
                }
                else if (action == TEXTx("8")) { /* Live View */
                    camera->get_live_view();
                }
                else if (action == TEXTx("9")) { /* Live View Image Quality */
                    camera->get_live_view_image_quality();
                    camera->set_live_view_image_quality();
                }
                else if (action == TEXTx("a")) { /* Position Key Setting */
                    camera->get_position_key_setting();
                    camera->set_position_key_setting();
                }
                else if (action == TEXTx("b")) { /* Exposure Program Mode */
                    camera->get_exposure_program_mode();
                    camera->set_exposure_program_mode();
                }
                else if (action == TEXTx("c")) { /* Still Capture Mode(Drive mode) */
                    camera->get_still_capture_mode();
                    camera->set_still_capture_mode();
                }
                else if (action == TEXTx("d")) { /* Focus Mode */
                    camera->get_focus_mode();
                    camera->set_focus_mode();
                }
                else if (action == TEXTx("e")) { /* Focus Area */
                    camera->get_focus_area();
                    camera->set_focus_area();
                }
                else if (action == TEXTx("11")) { /* FELock */
                    cli::tout << "Flash device required.";
                    camera->execute_lock_property((CrInt16u)SDK::CrDevicePropertyCode::CrDeviceProperty_FEL);
                }
                else if (action == TEXTx("12")) { /* AWBLock */
                    camera->execute_lock_property((CrInt16u)SDK::CrDevicePropertyCode::CrDeviceProperty_AWBL);
                }
                else if (action == TEXTx("13")) { /* AF Area Position(x,y) */
                    camera->set_af_area_position();
                }
                else if (action == TEXTx("14")) { /* Selected MediaFormat */
                    camera->get_select_media_format();
                    camera->set_select_media_format();
                }
                else if (action == TEXTx("15")) { /* Movie Rec Button */
                    camera->execute_movie_rec();
                }
                else if (action == TEXTx("16")) { /* White Balance */
                    camera->get_white_balance();
                    camera->set_white_balance();
                }
                else if (action == TEXTx("17")) { /* Custom WB */
                    camera->get_custom_wb();
                    camera->set_custom_wb();
                }
                else if (action == TEXTx("18")) { /* Zoom Operation */
                    camera->get_zoom_operation();
                    camera->set_zoom_operation();
                }
                else if (action == TEXTx("19")) { /* Remocon Zoom Speed Type */
                    camera->get_remocon_zoom_speed_type();
                    camera->set_remocon_zoom_speed_type();
                }
                else if (action == TEXTx("20")) { /* Preset Focus */
                    camera->execute_preset_focus();
                }
                else if (action == TEXTx("21")) { /*Remote menu 2*/
                    camera->remote_menu_2();
                }
#if defined(LIVEVIEW_ENB)
                else if (action == TEXTx("lv")) { /* LiveView Enable */
        ...
        else
        {
            ...
            while (true)
            {
                ...
                if (action == TEXTx("0")) { /* Return top menu */
                    if (camera->is_connected()) {
                        camera->disconnect();
                    }
                    break; // exit loop-D
                }
                else if (action == TEXTx("1")) { /* GetContentsList() */
                    if (camera->is_connected()) {
