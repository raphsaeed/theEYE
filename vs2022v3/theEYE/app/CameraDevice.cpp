...
SCRSDK::CrSdkControlMode CameraDevice::get_sdkmode() 
{
    ...
    if (SDK::CrSdkControlMode_ContentsTransfer == m_modeSDK) {
        tout << TEXTx("Contets Transfer Mode\n");
    }
    else {
        tout << TEXTx("Remote Control Mode\n");
    }
    return m_modeSDK;
}
    ...
    else
    {
        ...
        else
        {
            if (0 < image_data->GetSize())
            {
                ...
                path.append(TEXTx("LiveView000000.JPG"));
#endif
                tout << path << '\n';
...
bool CameraDevice::get_gain_base()
{
    load_properties();
    if (m_prop.gain_base_sensitivity.possible.size() < 1) {
        tout << "Gain Base is not supported\n";
        return false;
    }
...
bool CameraDevice::get_gain_base_iso()
{
    load_properties();
    if (m_prop.gain_base_iso_sensitivity.possible.size() < 1){
        tout << "Gain Base ISO is not supported \n";
        return false;
    }
...
bool CameraDevice::get_monitor_lut_setup()
{
    load_properties();
    if (m_prop.monitor_lut_setting.possible.size() < 1) {
        tout << "Monitor LUT is not supported \n";
        return false;
    }
    tout << "Monitor LUT: " << format_monitor_lut_setup(m_prop.monitor_lut_setting.current) << '\n';
    return true;
}
...
bool CameraDevice::get_movie_rec_button_toggle_enable_status()
{
    ...
    {
        tout << "Movie Rec Button(Toggle) Enable Status is not supported\n";
        return false;
    }
...
bool CameraDevice::set_save_info() const
{
    ...
    auto save_status = SDK::SetSaveInfo(m_device_handle
        , const_cast<text_char*>(path.data()), TEXTx(""), ImageSaveAutoStartNo);
#endif
...
void CameraDevice::set_af_area_position()
{
    ...
    if (1 != m_prop.focus_area.writable) {
        tout << "FocusArea is not writable\n";
        return;
    }
    SDK::CrDeviceProperty prop;
    prop.SetCode(SDK::CrDevicePropertyCode::CrDeviceProperty_FocusArea);
...
void CameraDevice::set_zoom_operation()
{
    ...
    while (true)
    {
        ...
        if (m_prop.zoom_speed_range.possible.size() < 2) {
            ...
            if (((input_value == 0) && (input != TEXTx("0"))) || (input_value < (int)m_prop.zoom_speed_range.possible.at(0)) || ((int)m_prop.zoom_speed_range.possible.at(1) < input_value))
            {
                cancel = true;
                ptpValue = SDK::CrZoomOperation::CrZoomOperation_Stop;
                tout << "Input cancelled.\n";
...
void CameraDevice::execute_camera_setting_reset()
{
    ...
    text input;
    tout << std::endl << "The camera settings will be initialized! Are you sure you want to do this ? (y/n):" << std::endl;
    std::getline(tin, input);
    if (input != TEXTx("y")) {
        tout << "Skip.\n";
        return;
...
void CameraDevice::set_gain_base()
{
    if (false == get_gain_base())
        return;
...
void CameraDevice::set_gain_base_iso()
{
    if (false == get_gain_base_iso())
        return;
...
void CameraDevice::set_monitor_lut_setup()
{
    if (false == get_monitor_lut_setup())
        return;
...
void CameraDevice::do_download_camera_setting_file()
{
    ...
    char* delimt = "/";
    if(strlen(path) + strlen(delimt) > MAC_MAX_PATH){
        tout << "Folder path is too long.\n";
        return;
    }
    strncat(path, delimt, strlen(delimt));
...
void CameraDevice::do_upload_camera_setting_file()
{
    ...
    int selected_index = 0;
    ss >> selected_index;
    if (selected_index<0|| selected_index> file_names.size()) {
        tout << "Input cancelled.\n";
        return;
    }
    ...
    char* delimt = "/";
    if(strlen(filename) + strlen(delimt) + file_names[selected_index].length() > MAC_MAX_PATH){
        tout << "Please shorten the file path. \n";
        return;
    }
    strncat(filename, delimt, strlen(delimt));
...
void CameraDevice::OnError(CrInt32u error)
{
    ...
    if (!msg.empty()) {
        ...
        if (SDK::CrError_Connect_SSH_ServerConnectFailed == error
            ...
            || SDK::CrError_Connect_SSH_PortfowardFailed == error
    ...
    if (prop_list && nprop > 0) {
        ...
        for (std::int32_t i = 0; i < nprop; ++i) {
            ...
            case SDK::CrDevicePropertyCode::CrDeviceProperty_GainBaseSensitivity:
                ...
                if (0 < nval) {
                    auto parsed_values = parse_gain_base_sensiyity(prop.GetValues(), nval);
            ...
            case SDK::CrDevicePropertyCode::CrDeviceProperty_GainBaseIsoSensitivity:
                ...
                if (0 < nval) {
                    auto parsed_values = parse_gain_base_iso_sensiyity(prop.GetValues(), nval);
...
void CameraDevice::getContentsList()
{
    ...
    if (CR_SUCCEEDED(err))
    {
        ...
        for (std::int32_t f_sep = 0; itF != m_foldList.end(); ++f_sep, ++itF)
        {
            ...
            tout << " (0x";
            std::ostringstream Fhandle_hex;
            Fhandle_hex << std::hex << std::uppercase << (*itF)->pFolder->handle;
            tout << std::dec;
            std::string Fhandle_str = Fhandle_hex.str();
            Fhandle_str.erase(std::remove(Fhandle_str.begin(), Fhandle_str.end(), ','), Fhandle_str.end());
            const char* Fhandle_char = Fhandle_str.c_str();
            tout.fill('0');
            tout.width(8);
            tout << Fhandle_char << ") , ";
            ...
            for (std::int32_t i = 0; itC != m_contentList.end(); ++i, ++itC)
            {
                if ((*itC)->parentFolderHandle == (*itF)->pFolder->handle)
                {
                    ...
                    tout << (i + 1);
                    tout << " : (0x";
                    std::ostringstream Chandle_hex;
                    Chandle_hex << std::hex << std::uppercase << (*itC)->handle;
                    tout << std::dec;
                    std::string Chandle_str = Chandle_hex.str();
                    Chandle_str.erase(std::remove(Chandle_str.begin(), Chandle_str.end(), ','), Chandle_str.end());
                    const char* Chandle_char = Chandle_str.c_str();
                    tout.fill('0');
                    tout.width(8);
                    tout << Chandle_char << "), ";
        ...
        while (1)
        {
            ...
            else
            {
                while (1)
                {
                    ...
                    int checkMax = 2;
                    if ((0 == ext.compare(TEXTx(".JPG"))) || 
                        (0 == ext.compare(TEXTx(".ARW"))) || 
                        (0 == ext.compare(TEXTx(".HIF"))))
                    {
                        checkMax = 3;
...
void CameraDevice::getThumbnail(SDK::CrContentHandle content)
{
    ...
    else
    {
        ...
        {
            text filename(TEXTx("Thumbnail.JPG"));
            if (fileType == SDK::CrFileType_Heif) {
                filename= (TEXTx("Thumbnail.HIF"));
            }
            ...
            char* delimt = "/";
            if(strlen(path) + strlen(delimt) + filename.length() > MAC_MAX_PATH){
                // FAILED
                delete[] image_buff; // Release
                delete image_data; // Release
                tout << "Failed to create save path\n";
...
void CameraDevice::execute_request_displaystringlist() {
    ...
    for (int i = 0; i <= SDK::CrDisplayStringType_Button_Assign_ShortDisplay; i++)
    {
        tout << "[" << i << "] " << format_display_string_type((SDK::CrDisplayStringType)i) << "\n";
    }
    tout << "[-1] Cancel input\n";
    tout << "input> ";
    text input;
    ...
    int selected_index = 0;
    ss >> selected_index;
    if (selected_index < 0 || SDK::CrDisplayStringType_Button_Assign_ShortDisplay < selected_index) {
        tout << "Input cancelled.\n";
        return;
    }
    type = (SDK::CrDisplayStringType)selected_index;
...
void CameraDevice::execute_get_displaystringtypes() {
    ...
    if (CR_SUCCEEDED(err)) {
        ...
        for (int i = 0; i < num; i++)
        {
            if (0 < Types[i] && Types[i] <= SDK::CrDisplayStringType::CrDisplayStringType_Button_Assign_ShortDisplay) {
                m_dispStrTypeList.push_back(Types[i]);
                tout << format_display_string_type(Types[i]) << "\n";
            }
        }
        tout << "----- ListType to be retrieved -----\n";
...
void CameraDevice::execute_get_displaystringlist() {
    if (m_dispStrTypeList.size() > 0) {
        ...
        int selected_index = 0;
        ss >> selected_index;
        if (selected_index < 0 || m_dispStrTypeList.size()< selected_index) {
            tout << "Input cancelled.\n";
            return;
        }
        ...
        if (CR_SUCCEEDED(err)) {
            tout << "\n";
            SDK::CrDisplayStringType listType = SDK::CrDisplayStringType_AllList;
            for (int i = 0; i < numOfList; i++)
            {
                if (0 < pProperty[i].listType && pProperty[i].listType <= SDK::CrDisplayStringType::CrDisplayStringType_Button_Assign_ShortDisplay) {

                    if (listType != pProperty[i].listType) {
                        listType = pProperty[i].listType;
                        tout << "----- " << format_display_string_type(listType) << " -----\n";
                    }
...
void CameraDevice::get_mediaprofile()
{
    ...
    if ((SDK::CrSlotStatus::CrSlotStatus_OK != m_prop.media_slot1_status.current) &&
        (SDK::CrSlotStatus::CrSlotStatus_OK != m_prop.media_slot2_status.current)) {
        tout << "\nMedia SLOT1/2 Status is Disable. Can not get media profile.\n";
        return;
    }

    tout << "\nSelect Get Mediaprofile SLOT. \n";
    tout << "[1] SLOT1\n";
    tout << "[2] SLOT2\n";
    tout << "[-1] Cancel input\n";
    tout << "input> ";
    ...
    {
        tout << "MediaProfile data is empty or GetMediaProfile is not supported.\n";
    }
}

void CameraDevice::CameraDevice::remote_menu_2() {
    while (1)
    {
        if (m_connected == false) {
            tout << "Disconnected\n";
        ...
        text input;
        cli::tout << "<< REMOTE-MENU 2 >>\nWhat would you like to do? Enter the corresponding number.\n";

        tout
            << "(0) Return to REMOTE-MENU 1\n"
            << "(1) Request Display String List\n"
            << "(2) Get Display String Type\n"
            << "(3) Get Display String List\n"
            << "(4) Iris Mode Setting\n"
            << "(5) Shutter Mode Setting\n"
            << "(6) ISO Current Sensitivity \n"
            << "(7) Exposure Control Type\n"
            << "(8) Gain Control Setting\n"
            << "(9) Gain Base Sensitivity \n"
            << "(10) Gain Base ISO Sensitivity \n"
            << "(11) Exposure Index \n"
            << "(12) BaseLook Value  \n"
            << "(13) Monitor LUT Setting \n"
            << "(14) Playback Media \n"
            << "(15) Download the camera setting file to PC\n"
            << "(16) Upload the camera setting file to Camera\n"
            << "(17) Camera Setting Reset \n"
            << "(18) APS-C/FULL setting \n"
            << "(19) Recording Setting\n"
            << "(20) DISP(Screen Disp) Setting\n"
            << "(21) Gain dB Value\n"
            << "(22) White Balance Tint\n"
            << "(23) Movie Rec Button(Toggle)\n"
            << "(24) Shutter Speed Value\n"
            << "(25) Get MediaProfile\n"
            << "(26) Set Focus Bracket Shot Num\n"
            << "(27) Set Focus Bracket Focus Range\n"
            << "(28) Focus Bracket Shot\n"
            ;
        ...
        int menu_index = 0;
        ss >> menu_index;
        if (menu_index < 1 || 28 < menu_index)
        {
            tout << "Return to REMOTE-MENU 1.\n";
            break;
        }
        else
        {
            ...
            if (1== menu_index) {
                execute_request_displaystringlist();
            }
            else if (2 == menu_index) {
                execute_get_displaystringtypes();
            }
            else if (3 == menu_index) {
                execute_get_displaystringlist();
            }
            else if (4 == menu_index) {/* Iris Mode Setting */
                set_iris_mode_setting();
            }
            else if (5 == menu_index) {/* Shutter Mode Setting */
                set_shutter_mode_setting();
            }
            else if (6 == menu_index) { /* ISO Current Sensitivity*/
                get_iso_current_sensitivity();
            }
            else if (7 == menu_index) { /* Exposure Ctrl Type*/
                set_exposure_control_type();
            }
            else if (8 == menu_index) {/* Gain Control Setting */
                set_gain_control_setting();
            }
            else if (9 == menu_index) { /* Gain Base Sensitivity */
                set_gain_base();
            }
            else if (10 == menu_index) {/* (Cine)Gain Base ISO Sensitivity */
                set_gain_base_iso();
            }
            else if (11 == menu_index) { /* Exposure Index*/
                set_exposure_index();
            }
            else if (12 == menu_index) { /* BaseLook Value */
                set_baselook_value();
            }
            else if (13 == menu_index) { /* Monitor LUT Setting */
                set_monitor_lut_setup();
            }
            else if (14 == menu_index) { /* Playback Media */
                set_playback_media();
            }
            else if (15 == menu_index) { /* camera_setting */
                do_download_camera_setting_file();
            }
            else if (16 == menu_index) { /* camera_setting */
                do_upload_camera_setting_file();
            }
            else if (17 == menu_index) { /* Camera Setting Reset */
                execute_camera_setting_reset();
            }
            else if (18 == menu_index) { /* APS-C or Full Switching */
                execute_APS_C_or_Full();
            }
            else if (19 == menu_index) { /* Recording Settting */
                set_recording_setting();
            }
            else if (20 == menu_index) { /* DisplayMode */
                set_dispmode();
            }
            else if (21 == menu_index) { /* Gain dB Value */
                set_gain_db_value();
            }
            else if (22 == menu_index) { /* White Balance Tint */
                set_white_balance_tint();
            }
            else if (23 == menu_index) { /* Movie Rec Button(Toggle) */
                execute_movie_rec_toggle();
            }
            else if (24 == menu_index) { /* Shutter Speed Value */
                set_shutter_speed_value();
            }
            else if (25 == menu_index) { /* Get MediaProfile */
                get_mediaprofile();
            }
            else if (26 == menu_index) { /* Set Focus Bracket Shot Num */
                set_focus_bracket_shot_num();
            }
            else if (27 == menu_index) { /* Set Focus Bracket Focus Range*/
                set_focus_bracket_focus_range();
            }
            else if (28 == menu_index) { /* Focus Bracket shooting */
                execute_focus_bracket();
            }
            cli::tout << std::endl;
        }
    }
